  ...
  <view class="container">
    ...
    <view class="content">
      ...
      <view class="card-box animate-card" :class="{ 'card-show': showCard }" :style="{ animationDelay: '0.1s' }">
        <view class="section-label">备忘录文案 <text class="required">*</text></view>
        <textarea
          class="memoTextarea"
          v-model="description"
          placeholder="请输入备忘录文案..."
          :maxlength="1000"
          :auto-height="true"
        />
        <view class="charCount" :class="{ 'limit-warn': description.length >= 980 }">
      ...
      <view class="card-box animate-card" :class="{ 'card-show': showCard }" :style="{ animationDelay: '0.2s' }">
        <view class="section-label">上传图片 <text class="required">*</text></view>
    ...
    <view class="success-page" v-if="isSubmitted">
      ...
      <view class="success-title animate-fadeup" :style="{ animationDelay: '0.3s' }">保存成功</view>
      <view class="success-desc animate-fadeup" :style="{ animationDelay: '0.5s' }">备忘录已成功保存</view>
...
export default {
  ...
  computed: {
    canSubmit() {
      return this.description.trim().length > 0 && this.imageList.length > 0 && !this.isSubmitting;
    },
  },
  onLoad() {
  ...
  methods: {
    ...
    async submitMemo() {
      ...
      if (!this.description.trim()) {
        uni.showToast({ title: "请输入备忘录文案", icon: "none" });
        return;
      }
      ...
      try {
        ...
        const uploadPromises = this.imageList.map((img) => {
          return api.uploadImage(img.path, {
            image_type: "memo",
